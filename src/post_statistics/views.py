...
class PostStatisticsViewSet(ApiErrorsMixin, ViewSet):
    def create(self, request: Request) -> Response:
        input_serializer = serializers.PostStatisticsSerializer(data=request.data)
    ...
    def retrieve_latest_by_post(self, request: Request) -> Response:
        query_params_serializer = serializers.PostStatisticsByPostQueryParamsSerializer(
            data=request.query_params
        )
        query_params_serializer.is_valid(raise_exception=True)
        output_serializer = serializers.PostStatisticsSerializer(
    ...
    @action(detail=False, methods=['get'])
    def list_latest_by_user(self, request: Request) -> Response:
